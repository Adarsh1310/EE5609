\documentclass[journal,12pt,twocolumn]{IEEEtran}
 ...
 \graphicspath{ {/user/adarshsrivastava/desktop/Matrix Theory/Assignemnt_7} }
  ...
  \begin{align}
  \myvec{x_1\\x_2\\x_3}^T\myvec{\alpha+\beta&\beta+\gamma&\gamma+\alpha}^T=0\\
   \myvec{x_1&x_2&x_3}\myvec{1&1&0\\0&1&1\\1&0&1}\myvec{\alpha^T\\\beta^T\\\gamma^T}=0\\
   \myvec{x_1+x_3&x_1+x_2&x_2+x_3}\myvec{\alpha^T\\\beta^T\\\gamma^T}=0\label{final}
   \end{align}
Since, $\vec{\alpha}$, $\vec{\beta}$ and $\vec{\gamma}$ are independent Hence \eqref{final} will have a trivial solution and so, ($\vec{\alpha}$+$\beta$),($\vec{\beta}$+$\vec{\gamma}$) and ($\vec{\gamma}$+$\vec{\alpha}$) are linearly independent.
\end{document}
