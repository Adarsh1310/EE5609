\documentclass[journal,12pt,twocolumn]{IEEEtran}
 ...
 \graphicspath{ {/user/adarshsrivastava/desktop/Matrix Theory/Assignemnt_7} }
  ...
  \end{abstract}
   ...
   Let's start by proving that $\vec{g}$ is linearly independent.
    ...
    \begin{align}
     \vec{v}^T\vec{B}=0
    \end{align}
    We can se that the rows of $\vec{B}$ are linearly independent and hence $\vec{v}^T$ will be zero vector.
  ...
  So,
  \begin{multline}
\vec{c}\vec{f}^T=\myvec{a\\0\\0}\vec{f}^T+\myvec{bt\\b\\0}\vec{f}^T+\myvec{ct^2\\2ct\\c}\vec{f}^T\end{multiline}
\begin{align}
\vec{c}\vec{f}^T=\left(\myvec{a\\0\\0}+\myvec{bt\\b\\0}+\myvec{ct^2\\2ct\\c}\right)\vec{f}^T\\
\myvec{c_0\\c_1\\c_2}\vec{f}=\myvec{a+bt+ct^2\\b+2ct\\c}\vec{f}^T
  \end{align} 
