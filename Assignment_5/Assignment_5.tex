...
Now,Subtracting equation \eqref{2.2.2} from \eqref{2.1.2} We get,
 ...
 \begin{align}
 \myvec{-1&0}\vec{x}=-1\\
 \vec{x} = \lambda \vec{m}+q, \text{where} \vec{m} = \myvec{0\\1}-1\label{2.2.9}\\
 \vec{x} = \lambda \myvec{0\\1}-1\label{2.2.10}
 \end{align}
 Substituting \eqref{2.2.9} in \eqref{2.2.2}
  \begin{align}
  {\vec{x}^{T}\vec{x}+2\vec{u_2}^T\vec{x}}=0\\
(q+\lamda\vec{m})^2(q+\lamda{m})+2\vec{u}^T(q+\lembda\vec{m})=0
\end{align}
Substituting the value of $\lambda$,$\vec{u}$ and q and solving\\
\begin{align}
\lambda^2-4+1=0\\
\lambda^2-3=0\\
\lambda^2=3\\
\lambda=\sqrt{3},-\sqrt{3}
 \end{align}
Substituting value of $\lambda$ in \eqrt{2.2.10},Points of intersection come out to be(-1,\sqrt{3}) and(-1,-\sqrt{3}),$Now finding the direction vector between point of intersection and origin of two circle.
