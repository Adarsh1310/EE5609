...
Let,\begin{align*} \norm{\vec{X}}_2=\sqrt{\norm{\vec{X}_{1}^{2}}+\cdots+\norm{\vec{X}_{n}^{2}}}\end{align*} for $\vec{X}$=$(\vec{X}_{1},\cdots,\vec{X}_{n})\in \mathbb{C}^n$. If \begin{align*}
p(\vec{A})=a_0\vec{I}+a_1\vec{A}+\cdots+a_n\vec{A}^n
\end{align*}
then $sup_{\norm{\vec{X}_2=1}}\norm{p(\vec{A})}_2$ is equal to
...
\begin{align}
\norm{p(A)}_{2}=\rho{(p(\vec{A}))}\\
=max\{\norm{\alpha}: \alpha $ is the eigen values of p(A)$\}\\
=max\{\norm{p(\lambda_j)}:j=1, 2, \cdots n\}\\
=max\{\norm{a_0+a_1\lambda_j+\cdots+a_n{\lambda_j}^n}: j=1, 2, \cdots n\}
\end{align}
\end{document}
