...
\begin{align}
\vec{m}_{AC}\vec{m}_{BE}=0\\
\vec{(A-C)}\vec{(B-E)}=0\label{2.0.4}
\end{align}
...
\begin{align}
\vec{m}_{AB}\vec{m}_{CF}=0\\
\vec{(A-B)}\vec{(C-F)}=0\label{2.0.8}
\end{align}
Comparing \eqref{2.0.4} and \eqref{2.0.8}
\begin{align}
\vec{(A-C)}\vec{(B-E)}=\vec{(A-B)}\vec{(C-F)}\\
\vec{(B-E)}=\vec{(C-F)} [\because \vec{m}_{AB}=\vec{m}_{AC}]\\
\vec{m}_{BE}=\vec{m}_{CF}
\end{align}
Hence,we can say that these altitudes are equal.
\end{document}