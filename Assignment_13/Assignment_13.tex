...
\begin{align}
\implies \vec{A}k=0\label{Ak}
\end{align}
For \eqref{Ak} to be true k should be zero. We also know that $\vec{B}$ is 1 $\times$ 2 matrix i.e. rows are less than column hence,\begin{align} \vec{B}\vec{x}=0\label{bx} \end{align}will have a non trivial solution. Hence, using \eqref{Ak} and \eqref{bx} we can say,
\begin{align}
\vec{A}\vec{B}\vec{x}=0
\end{align} 
