...
Now,for vector addition,
   \begin{enumerate}
 \item{Since addition in $\mathbb{V}$ is commutative}
 \begin{align}
 f(t)+g(t)=g(t)+f(t)
 \end{align}
 for all t, so the functions f+g and g+f are equal.
 \item{Since addition in $\mathbb{V}$ is associative}
 \begin{align}
 (f(t)+g(t))+a(t)=f(t)+(g(t)+a(t))
 \end{align}
 for all t, so the functions f+(g+a) and (f+g)+a are equal.
\item{The unique zero vector is the zero function which assigns to each element, the scalar 0 in $\mathbb{V}$.
\item{For each f in $\mathbb{V}$,(-f) is the function which is given by}
(-f)(s)=-f(s).
\end{enumerate}
Now, for scalar multiplication
\begin{enumerate}
\item{Since scalar multiplication is associative}
\begin{align}
a(bf(t))=ab(f(t))
\end{align}
for all a and b, the functions (ab)f and a(bf) are equal.
\item{Since scalar multiplication is distributive}
\begin{align}
a(f(t)+g(t))=af(t)+ag(t)
\end{align}
for all a, the functions a(f+g)=af+ag.
\item{Since scalar addition is distributive}
\begin{align}
(a+b)f(t)=af(t)+bf(t)
\end{align}
for all a and b, the functions (a+b)f=af+bf.
\item{The unique scalar constant 1 which multiplied to any vector in $\mathbb{V}$ will return the same vector.
\end{enumerate}
