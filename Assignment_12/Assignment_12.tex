...
Now multiplying $\vec{A}$ and $\vec{X}$ and substituting \eqref{d}.
  ...
  =\myvec{\vec{I}&\vec{a}\\\vec{I} &\vec{b}}\vec{X}
  \end{align}
  Now using, $\vec{X}^T$$\vec{A}^T$
  \begin{align}
  =\myvec{\vec{y}^T&z}\myvec{\vec{I}&\vec{I} \\\vec{a}^T&\vec{b}^T}\\
  =\myvec{\vec{y}^T+z\vec{a}^T&\vec{y}+z\vec{b}^T}\label{ax}
  \end{align}
  Since \eqref{ax} is coming out to be a zero vector. We can say that $\vec{R}$ and $\vec{R}^{'}$ are linear combination of each other.
  Also from \eqref{49} and \eqref{50}
  \begin{align}
    \vec{y}^T+z\vect{a}^T=\vec{y}^T+z\vect{b}^T\\
    \vect{a}^T=\vect{b}^T\\
    b=e\\
    c=f
  \end{align}
